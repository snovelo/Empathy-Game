    ...
    <main className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-50 py-8 px-4">
      <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="text-center py-6">
          ...
          <h1 className="text-3xl font-extrabold text-gray-900 mb-2">Game Over!</h1>
          <p className="text-gray-500">Thanks for playing {session.title}</p>
        </div>

        {/* Winner banner */}
        {leaderboard.length > 0 && (
          <div className="bg-gradient-to-r from-brand-500 to-amber-500 rounded-2xl p-6 text-white text-center shadow-lg">
            <p className="text-2xl mb-1">🏆</p>
            <p className="text-xl font-bold">{leaderboard.sort((a, b) => b.totalScore - a.totalScore)[0]?.teamName}</p>
            <p className="text-white/80 text-sm font-medium mt-0.5">Top <span className="text-amber-200 font-bold">GOLD</span> Standard Team</p>
          </div>
        )}
              ...
              {myRoundScores.map(({ round, submission }) => (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-sm text-gray-700 truncate">{round.title}</span>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <span className={cn('text-sm font-bold tabular-nums', scoreColor(submission!.finalScore ?? 0))}>
                      ...
                      {round.isBonus && (
                        <span className="text-xs font-normal text-amber-500 ml-0.5">×2</span>
                      )}
                    </span>
                  </div>
                </div>
        ...
        {/* Team Leaderboard */}
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
          <h2 className="font-bold text-gray-900 mb-4">Final Team Leaderboard</h2>
          <Leaderboard
            entries={leaderboard}
        ...
        {/* Round summary */}
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
          <h2 className="font-bold text-gray-900 mb-4">Round Summary</h2>
              ...
              .map((sr) => {
                const avg =
                  sr.submissions.length > 0
                    ...
                    : null;
                return (
                  <div key={sr.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                    <div className="flex items-center gap-2">
                      {sr.round.isBonus && <Badge variant="bonus">⚡</Badge>}
                      <span className="text-sm text-gray-700">{sr.round.title}</span>
                    </div>
                    <div className="text-right">
